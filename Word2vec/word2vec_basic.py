...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  ops=0
  flag_ops=1
  win_size = 2 * skip_window + 1 
  m = skip_window
  rep=[]
  win_start = data_index
  win_end = (data_index + win_size) % len(data) 
  
  while batch_idx < batch_size:
    registry = [m] 
    pointer = m

    for k in range(0, num_skips):
      while pointer in registry:
        pointer = random.randint(win_start, win_start + win_size) % len(data) 
        if( pointer in rep): 
          continue
      registry.append(pointer) 
      labels[batch_idx] = data[pointer]
      batch[batch_idx] = data[m]
      #print(batch)
      batch_idx+=1 
    m += 1
    win_start = (win_start + 1) % len(data) 
    win_end = (win_end + 1) % len(data)
