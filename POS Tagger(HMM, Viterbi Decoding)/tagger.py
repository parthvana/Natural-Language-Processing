...
class CRFPerceptron(Tagger):
    ...
    def __init__(self, feats = Feats()):
        self.feats = feats
        self.le = preprocessing.LabelEncoder()
        self.cls = struct_perceptron.StructuredPerceptron(self, max_iter=25,decay_exponent=0.5, decay_t0=7, average=True, verbose=True)
    ...
    def joint_feature(self, Xs, ys):
        ...
        # init_trans
        fv[0,int(self.get_start_trans_idx(ys[0]))] = 1
        # final_trans
        fv[0,int(self.get_end_trans_idx(ys[-1]))] = 1
        # intermediate transitions
        for i in xrange(1, len(ys)):
            tidx = int(self.get_trans_idx(ys[i-1], ys[i]))
